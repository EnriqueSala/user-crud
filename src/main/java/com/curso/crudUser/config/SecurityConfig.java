...
public class SecurityConfig extends WebSecurityConfigurerAdapter{
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		...
		auth.inMemoryAuthentication()
			...
			.withUser(users.username("Enrique").password("test123").roles("EMPLOYEE"))
			.withUser(users.username("Miguel").password("test123").roles("EMPLOYEE", "MANAGER"))
			.withUser(users.username("Patrobas").password("test123").roles("EMPLOYEE", "ADMIN"));
